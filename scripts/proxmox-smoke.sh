...
if [ ! -f /var/lib/qubes-provision.done ]; then
  ...
  # placeholder wg0.conf + the user-run wg-setup-key helper, but must NEVER bake a
  # private key OR auto-generate one at boot. CI boots the very qcow2 it publishes
  # during this smoke test, so any first-boot keygen would splice a real key into
  # the public artifact -- hence NO systemd keygen unit, no keys on disk at all.
  # wg-quick@wg0 must be DISABLED (no server pubkey yet, would fail-loop on boot).
  ...
  [ -e /etc/wireguard/publickey ] \
    && fail "/etc/wireguard/publickey exists -- keygen ran during the smoke boot (would leak the matching privkey)"
  # There must be NO auto-keygen unit -- keys are generated by the user-run helper only.
  [ -e /etc/systemd/system/wg-genkey.service ] \
    && fail "wg-genkey.service present -- auto-keygen at boot leaks a private key into the published image"
  SETUPKEY=/usr/sbin/wg-setup-key
  [ -x "${SETUPKEY}" ] || fail "missing ${SETUPKEY} (user-run WG keygen helper)"
  sh -n "${SETUPKEY}" || fail "${SETUPKEY} has a syntax error"
  ...
  echo "ok: WireGuard peer scaffolding present (no baked/auto-generated secret, wg-setup-key shipped, wg-quick disabled)"
